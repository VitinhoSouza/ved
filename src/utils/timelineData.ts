type TimelineItem = {
  date: string;
  title: string;
  description: string;
  images: string[];
};

import primeiroBeijo from "../assets/timeline/primeiro-beijo.jpg";
import inicio from "../assets/timeline/inicio.jpg";
import inicio2 from "../assets/timeline/inicio-2.jpg";
import inicio3 from "../assets/timeline/inicio-3.jpg";

import pedidoNamoro from "../assets/timeline/pedido-namoro.jpg";
import anos17 from "../assets/timeline/17-anos.jpg";
import primeiroNamorados from "../assets/timeline/primeiro-namorados.jpg";

import primeiraViagem from "../assets/timeline/primeira-viagem.jpg";
import primeiraViagem2 from "../assets/timeline/primeira-viagem-2.jpg";

import umAno from "../assets/timeline/1ano.jpg";
import lucinete50 from "../assets/timeline/lucinete-50anos.jpg";
import universalPark from "../assets/timeline/universal-park.jpg";
import primeiroRevellion from "../assets/timeline/primeiro-revellion.jpg";

import anos18Festa from "../assets/timeline/18-anos-festa.jpg";
import anos18Praia from "../assets/timeline/18-anos-praia.jpg";
import anos18Biquini from "../assets/timeline/18-anos-biquini.jpg";

import victoria4 from "../assets/timeline/victoria-4anos.jpg";
import viagemJeri from "../assets/timeline/viagem-jeri.jpg";
import viagemGuaramiranga from "../assets/timeline/viagem-guaramiranga.jpg";
import copaMundo from "../assets/timeline/copa-mundo.jpg";
import natal2022 from "../assets/timeline/natal-2022.jpg";
import revellion2022 from "../assets/timeline/revellion-2022.jpg";

import anos19BM from "../assets/timeline/19anos-bm.jpg";
import cafeFlores from "../assets/timeline/cafe-flores.jpg";

import namoradosParque from "../assets/timeline/namorados-parque.jpg";
import namoradosParque2 from "../assets/timeline/namorados-parque-2.jpg";

import arraia2023 from "../assets/timeline/arraia-tarcisa-2023.jpg";

import morroBranco from "../assets/timeline/viagem-morro-branco.jpg";
import canoaQuadriciclo from "../assets/timeline/viagem-canoa-quadriciclo.jpg";
import canoaVila from "../assets/timeline/viagem-canoa-vila.jpg";

import piauiPortinho from "../assets/timeline/viagem-piaui-portinho.jpg";
import dafny2 from "../assets/timeline/dafny-2anos.jpg";
import viagemMoto from "../assets/timeline/primeira-viagem-moto.jpg";
import laSuite from "../assets/timeline/meuniver-lasuite.jpg";

import natalAquiraz2023 from "../assets/timeline/natal-2023-aquiraz.png";
import revellionParnaiba from "../assets/timeline/viagem-revellion-2023.png";
import minhaFormatura from "../assets/timeline/minha-formatura.jpg";
import viagemTaiba from "../assets/timeline/viagem-taiba.jpg";
import anos20 from "../assets/timeline/20-anos-saomiguel.jpg";

import aguasCristalinas from "../assets/timeline/aguas-cristalinas.jpg";

import natalAquario from "../assets/timeline/viagem-natal-aquario.jpg";
import natalPipa from "../assets/timeline/viagem-natal-pipa.jpg";

import casalFitness from "../assets/timeline/casal-fitness.jpg";
import guaramiranga2024 from "../assets/timeline/viagem-guaramiranga-2024.jpg";
import natal2024 from "../assets/timeline/natal-2024-saomiguel.jpg";
import revellion2024 from "../assets/timeline/revellion-2024-juatama.jpg";

import aquiraz2025 from "../assets/timeline/viagem-aquiraz-2025.jpg";
import aurora1 from "../assets/timeline/niver-aurora.jpg";

import flecheirasPraia from "../assets/timeline/viagem-flecheiras-praia.jpg";
import flecheirasRio from "../assets/timeline/viagem-flecheiras-rio.jpg";

import arraia2025 from "../assets/timeline/arraia-tarcisa-2025.jpg";

import casa1 from "../assets/timeline/casa.jpg";
import casa2 from "../assets/timeline/casa-2.jpg";
import casa3 from "../assets/timeline/casa-3.jpg";

export const timelineData: TimelineItem[] = [
  {
    date: "07/01/2020",
    title: "Primeiro beijo 😘",
    description:
      "Um momento que mudou tudo, cheio de risadas e olhares tímidos.",
    images: [primeiroBeijo],
  },
  {
    date: "",
    title: "Nosso início...",
    description:
      "No começo, nossa situação era difícil, pois se víamos pouco e se falávamos mais por celular, tanto por causa da distância como pelo fato de eu não possuir transporte. Quando se víamos, nossa opção era ficar em casa, assistindo série ou fazendo o tempo passar de outra forma.",
    images: [inicio, inicio2, inicio3],
  },
  {
    date: "20/08/2020",
    title: "Pedido de namoro 👩‍❤️‍👨",
    description:
      "Aquele dia em que eu, envergonhado, visitei sua casa pela primeira vez e pedi a seus pais para namorar com você.",
    images: [pedidoNamoro],
  },
  {
    date: "26/03/2021",
    title: "Seus 1️⃣7️⃣ anos 🎉",
    description:
      "O dia que eu te 'enganei' falando que não iria poder ir, mas fiz o esforço de ir e participar desse momento muito importante para você.",
    images: [anos17],
  },
  {
    date: "12/06/2021",
    title: "Primeiro dia dos namorados 👩‍❤️‍👨",
    description:
      "Nosso primeiro Dia dos Namorados juntos… uma data simples, mas cheia de significado.",
    images: [primeiroNamorados],
  },
  {
    date: "13/08/2021",
    title: "Primeira viagem juntos 🛣️",
    description:
      "Nossa primeira viagem juntos para Piauí. Dias especiais, cheios de carinho, risadas e novas histórias que ficaram guardadas na memória.",
    images: [primeiraViagem, primeiraViagem2],
  },
  {
    date: "20/08/2021",
    title: "1️⃣ ano de namoro ❤️",
    description:
      "Comemoramos 1 ano de namoro do nosso jeito: rindo e virando crianças nos brinquedos do shopping. Um ano de amor, parceria e muita diversão!",
    images: [umAno],
  },
  {
    date: "11/10/2021",
    title: "Aniversário da Lucinete 🎂",
    description:
      "Nesse dia, comemoramos os 50 anos da sua tia Lucinete. Um dia marcante, onde pude conhecer várias pessoas da família e ainda aproveitamos o resto da noite em um 'piseiro' com sua família.",
    images: [lucinete50],
  },
  {
    date: "22/10/2021",
    title: "Primeira vez no Universal Park 🎡",
    description:
      "Nossa primeira vez juntos no Universal Park… nem imaginávamos que aquele dia se tornaria um costume nosso. Cada volta, cada risada e cada momento ali só reforça o quanto é bom dividir a vida contigo.",
    images: [universalPark],
  },
  {
    date: "01/01/2022",
    title: "Primeiro Réveillon juntos 🎆",
    description:
      "Viramos um ano juntinhos pela primeira vez e começamos 2022 cheios de sonhos, planos e promessas para o futuro.",
    images: [primeiroRevellion],
  },
  {
    date: "26/03/2022",
    title: "Seus 1️⃣8️⃣ anos 🎂",
    description:
      "Uma data marcante, cheia de sorrisos e muitas pessoas queridas. Um aniversário especial para lembrar sempre.",
    images: [anos18Festa, anos18Praia, anos18Biquini],
  },
  {
    date: "17/04/2022",
    title: "Aniversário da Victoria 🎂🎈",
    description:
      "Nesse dia, comemoramos os 4 anos da Victoria. Um dia muito divertido que podemos aproveitar juntos!",
    images: [victoria4],
  },
  {
    date: "20/08/2022",
    title: "Comemorando 2️⃣ anos de namoro em Jeri 🏝️",
    description:
      "Dias de sol, areia e mar em Jeri… momentos únicos vivendo a liberdade e a paz ao seu lado.",
    images: [viagemJeri],
  },
  {
    date: "20/11/2022",
    title: "Viagem para Guaramiranga 🌲",
    description:
      "Entre o friozinho da serra e boas conversas, aproveitamos juntos um clima diferente e acolhedor.",
    images: [viagemGuaramiranga],
  },
  {
    date: "24/11/2022",
    title: "Primeira Copa do Mundo juntos ⚽",
    description:
      "Torcendo, vibrando e sofrendo lado a lado… nossa primeira Copa juntos foi cheia de emoção.",
    images: [copaMundo],
  },
  {
    date: "24/12/2022",
    title: "Natal 2022 🎄",
    description:
      "Um Natal especial, dividindo tradições, carinho e a alegria de estar ao seu lado nessa época mágica.",
    images: [natal2022],
  },
  {
    date: "01/01/2023",
    title: "Réveillon 2023 ✨",
    description:
      "Mais um ano começando juntos, brindando nossos sonhos e desejando um futuro ainda mais lindo.",
    images: [revellion2022],
  },
  {
    date: "26/03/2023",
    title: "Seus 1️⃣9️⃣ anos na BM 🎉",
    description:
      "Um aniversário cheio de energia, risadas e pessoas queridas para comemorar ao seu lado.",
    images: [anos19BM],
  },
  {
    date: "28/04/2023",
    title: "Café com flores ☕🌹",
    description:
      "O dia em que demos uma saidinha para visitar o Café com flores em Quixeramobim e de quebra ainda levei minha primeira multa na moto 😅",
    images: [cafeFlores],
  },
  {
    date: "12/06/2023",
    title: "Dia dos namorados no parque 🎢",
    description:
      "Entre frio na barriga e muitas risadas na roda gigante, sentimos medo juntos, mas transformamos tudo em diversão e boas lembranças.",
    images: [namoradosParque, namoradosParque2],
  },
  {
    date: "24/06/2023",
    title: "Arraiá da Tarcisa 2023",
    description:
      "Celebramos em família mais um ano de vida da minha vó, rodeados de carinho e boas memórias.",
    images: [arraia2023],
  },
  {
    date: "11/07/2023",
    title: "Viagem Morro Branco & Canoa Quebrada 🏖️",
    description:
      "Exploramos juntos as paisagens incríveis e as falésias coloridas, colecionando lembranças inesquecíveis, além de desfrutar bastante da vila. Essa viagem, ainda que antecipada, marcou nossos 3 anos de namoro.",
    images: [morroBranco, canoaQuadriciclo, canoaVila],
  },
  {
    date: "05/08/2023",
    title: "Lagoa do Portinho - Piauí 🌊",
    description:
      "Mais uma aventura no Piauí, com águas tranquilas e momentos de pura leveza.",
    images: [piauiPortinho],
  },
  {
    date: "19/08/2023",
    title: "Aniversário da Dafny 🎂🎈",
    description:
      "Lembrando da festa de 2 anos da Dafny. Um dia cheio de sorrisos, brincadeiras e momentos que ficaram guardados com carinho.",
    images: [dafny2],
  },
  {
    date: "21/08/2023",
    title: "Primeira viagem de moto 🏍️",
    description:
      "Sentindo o vento no rosto, nossa primeira viagem de moto foi cansativa, mas com muita liberdade e parceria.",
    images: [viagemMoto],
  },
  {
    date: "22/10/2023",
    title: "Um dia de lazer no La Suite🍹",
    description: "Curtindo um dia no La Suite junto com amigos especiais.",
    images: [laSuite],
  },
  {
    date: "24/12/2023",
    title: "Natal em família em Aquiraz 🎄",
    description:
      "Um Natal rodeado de família, união e muito amor na linda cidade de Aquiraz.",
    images: [natalAquiraz2023],
  },
  {
    date: "01/01/2024",
    title: "Réveillon em Parnaíba - PI 🎆",
    description:
      "Viramos o ano no Piauí, celebrando juntos com muita alegria e energia positiva a chegada de 2024.",
    images: [revellionParnaiba],
  },
  {
    date: "22/01/2024",
    title: "Minha formatura 🎓",
    description:
      "Um marco especial: a conquista da formatura celebrada lado a lado, cheia de orgulho e emoção.",
    images: [minhaFormatura],
  },
  {
    date: "13/02/2024",
    title: "Conhecendo Taíba 🌊",
    description:
      "Descobri contigo a beleza da Taíba, entre ondas, vento e momentos inesquecíveis.",
    images: [viagemTaiba],
  },
  {
    date: "26/03/2024",
    title: "Seus 2️⃣0️⃣ anos 🎉",
    description:
      "Mais um ano de vida comemorado com alegria, carinho e amor do jeitinho que você merece.",
    images: [anos20],
  },
  {
    date: "16/06/2024",
    title: "Passeio nas Águas Cristalinas 🌊☀️",
    description:
      "Um dia de sol, risadas e mergulhos nas águas cristalinas de Cumbuco… momentos simples que se tornam inesquecíveis quando estamos juntos.",
    images: [aguasCristalinas],
  },
  {
    date: "17/08/2024",
    title: "Viagem para Natal - RN 🏖️",
    description:
      "Exploramos juntos as praias e a cidade incrível que é Natal, aproveitando cada momento como se fosse único e comemorando nossos 4 anos de namoro.",
    images: [natalAquario, natalPipa],
  },
  {
    date: "14/11/2024",
    title: "Casal fitness 🏋️‍♂️❤️",
    description:
      "Aquele dia em que, além de treinar, a gente decidiu eternizar o shape e a brincadeira.",
    images: [casalFitness],
  },
  {
    date: "01/12/2024",
    title: "Viagem para Guaramiranga 🍃",
    description:
      "Mais uma ida para a serra, curtindo o friozinho e bons momentos juntinhos.",
    images: [guaramiranga2024],
  },
  {
    date: "25/12/2024",
    title: "Natal 2024 🎄✨",
    description:
      "Passamos mais um Natal juntos, celebrando em família e espalhando amor.",
    images: [natal2024],
  },
  {
    date: "01/01/2025",
    title: "Réveillon 2025 🎆",
    description:
      "Mais um ano começando, desta vez em Juatama, cheio de boas energias e união.",
    images: [revellion2024],
  },
  {
    date: "03/01/2025",
    title: "Viagem Aquiraz em família 🌊",
    description:
      "Dias incríveis em Aquiraz, curtindo a praia e a companhia especial da família.",
    images: [aquiraz2025],
  },
  {
    date: "11/01/2025",
    title: "1 ano da Aurora ✨🎂",
    description:
      "Um dia cheio de amor e sorrisos, celebrando essa sobrinha tão especial que cuidamos e amamos tanto. Foi demais ver tudo lindo na festa dela!",
    images: [aurora1],
  },
  {
    date: "12/05/2025",
    title: "Viagem para Flecheiras 🏝️",
    description:
      "Descobrindo as belezas de Flecheiras, entre mar, sol e momentos únicos ao seu lado.",
    images: [flecheirasPraia, flecheirasRio],
  },
  {
    date: "21/06/2025",
    title: "Arraiá da Tarcisa 2025 🎊🌽",
    description:
      "Dia de celebrar o aniversário da vó com direito a música, comida boa e, o melhor de tudo, família reunida dos dois lados. Foi especial demais!",
    images: [arraia2025],
  },
  {
    date: "18/07/2025",
    title: "O dia em que começamos a morar juntos 🏡💛",
    description:
      "Um marco na nossa história, cheio de expectativa, frio na barriga e a certeza de que construiríamos nosso cantinho lado a lado.",
    images: [casa3, casa2, casa1],
  },
];
